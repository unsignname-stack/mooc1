...
class GroupAdmin(object):
    ...
    # style_fields = {'permissions': 'checkbox-inline'}
...
class UserAdmin(object):
    ...
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_staff','mobile','image','gender','birthday')
    ...
    list_editable =('email', 'first_name', 'last_name','mobile','image')
    def queryset(self):
        qs = super(UserAdmin, self).queryset()
        if self.request.user.is_superuser:  # 超级用户可查看所有数据
            return qs
        else:
            return qs.filter(username=self.request.user)  # username是user Model的用户字段
        # super().queryset(self)
